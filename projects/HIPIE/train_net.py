...
from projects.hipie.data import build_detection_train_loader, build_detection_test_loader
#from detectron2.projects.hipie import add_hipie_config
from projects.hipie import add_hipie_config
...
#)
from projects.hipie.data import (
    get_detection_dataset_dicts, DetrDatasetMapperUni, YTVISDatasetMapper, YTVISEvaluator, SOTDatasetMapper, UniVidDatasetMapper
)
import logging
...
# Unification
from detectron2.projects.hipie.data import build_custom_train_loader
from detectron2.evaluation.d2_evaluator import COCOPanopticEvaluator,SemSegEvaluator
from detectron2.evaluation.d2_evaluator import COCOEvaluator as COCOEvaluatorODISE
from detectron2.projects.hipie.data.evaluation.instance_evaluation import InstanceSegEvaluator
from detectron2.evaluation.sem_seg_evaluation import SemSegEvaluatorParts
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        elif evaluator_type == "coco_panoptic_seg":
            evaluator_list.append(COCOEvaluatorODISE(dataset_name=dataset_name))
            evaluator_list.append(COCOPanopticEvaluator(dataset_name=dataset_name))
            evaluator_list.append(SemSegEvaluator(dataset_name=dataset_name))
        elif evaluator_type == "ade20k_panoptic_seg":
            evaluator_list.append(InstanceSegEvaluator(dataset_name, output_dir=output_folder))
            evaluator_list.append(COCOPanopticEvaluator(dataset_name=dataset_name))
            evaluator_list.append(SemSegEvaluator(dataset_name=dataset_name))
        elif evaluator_type == 'seginw':
            evaluator_list.append(InstanceSegEvaluator(dataset_name, output_dir=output_folder))
        elif evaluator_type == 'odinw':
            evaluator_list.append(InstanceSegEvaluator(dataset_name,tasks=['bbox'], output_dir=output_folder))
        elif evaluator_type == "sem_seg":
            if 'parts' in dataset_name:
                evaluator_list.append(SemSegEvaluatorParts(dataset_name=dataset_name))
            else:
                evaluator_list.append(SemSegEvaluator(dataset_name=dataset_name))
    ...
    def build_optimizer(cls, cfg, model):
        ...
        def maybe_add_full_model_gradient_clipping(optim):  # optim: the optimizer class
            ...
            grad_acc_iter = cfg.SOLVER.ACC_ITER
            enable = (
            ...
            class FullModelGradientClippingOptimizer(optim):
                ...
                def __init__(self,*args,**kwargs):
                    ...
                    self.grad_acc_factor = 1.0 / self.grad_acc_iter
                    self.steps_curr = 0

                def scale_norm(self,parameters):
                    if self.grad_acc_iter == 1:
                        return
                    ...
                    if len(parameters) == 0:
                        return
                    for p in parameters:
                        p.grad.detach().mul_(self.grad_acc_factor)

                def zero_grad(self, set_to_none: bool = False):
                    if self.grad_acc_iter == 1 or self.steps_curr % self.grad_acc_iter == 1 or self.steps_curr  == 0:
                        super().zero_grad(set_to_none=set_to_none)
                def step(self, closure=None):
                    if self.grad_acc_iter == 1 or self.steps_curr % self.grad_acc_iter == 0:
                        all_params = itertools.chain(*[x["params"] for x in self.param_groups])
                        self.scale_norm(all_params)

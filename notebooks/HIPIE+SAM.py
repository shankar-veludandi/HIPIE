...
!git clone https://github.com/shankar-veludandi/HIPIE
%cd HIPIE

# Clone the YouTube-VOS cocoapi repository
!git clone https://github.com/youtubevos/cocoapi.git

# Navigate to the PythonAPI directory
%cd cocoapi/PythonAPI

# Download ytvos.py and ytvoseval.py
!wget https://github.com/youtubevos/cocoapi/raw/master/PythonAPI/pycocotools/ytvos.py
!wget https://github.com/youtubevos/cocoapi/raw/master/PythonAPI/pycocotools/ytvoseval.py

# Copy ytvos.py and ytvoseval.py to the pycocotools directory in site-packages
...
%cd /content/HIPIE/projects/HIPIE/hipie/models/maskdino/pixel_decoder/ops
!python setup.py build_ext --inplace

# Navigate back to the project root (if necessary)
%cd /content/HIPIE

# Add the module to the Python path
import sys
sys.path.append('/content/HIPIE/projects/HIPIE/hipie/models/maskdino/pixel_decoder/ops')
...
# Remove the potentially corrupted checkpoint file
!rm weights/sam_vit_h_4b8939.pth

# Re-download the checkpoint file
from huggingface_hub import hf_hub_download
hf_hub_download(repo_id="n0r9st/segment-anything", filename="sam_vit_h_4b8939.pth", local_dir="weights")
...
# Create the directory for the BERT-base model
!mkdir -p /content/HIPIE/projects/HIPIE/bert-base-uncased

# Change to the directory
%cd /content/HIPIE/projects/HIPIE/bert-base-uncased
...
predictions, visualized_output = demo.run_on_image(img, 0.5, args.task, dataset_name='coco_panoptic', **test_args_custom)
with torch.no_grad():
    panoptic_seg, segments_info = predictions['panoptic_seg']
panoptic_seg = panoptic_seg.cpu()
sem_seg2 = predictions['sem_seg']

predictions, visualized_output = demo_parts.run_on_image(img, 0.5, args.task, None, **get_args_eval())
